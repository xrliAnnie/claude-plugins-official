...
describe("buildRoundtableThreadCreateBody (FLY-1435)", () => {
	...
	for (const [label, value] of [
		...
		test(`${label} parent default refuses to build a silent 4320 POST body`, async () => {
			const payload =
				label === "missing"
					? {}
					: { default_auto_archive_duration: value };
	...
	for (const status of [401, 403, 404, 429, 500, 503]) {
		test(`HTTP ${status} refuses to build a silent 4320 POST body`, async () => {
			const { body, warns } = await createBody(
	...
	test("malformed JSON refuses to build a silent 4320 POST body", async () => {
		const { body, warns } = await createBody(
			(async () =>
				({
					ok: true,
	...
	test("network rejection refuses to build a silent 4320 POST body", async () => {
		const { body, warns } = await createBody(
			(async () => {
	...
	test("timeout is bounded and refuses to build a silent 4320 POST body", async () => {
